...
type Version struct {
	...
	// canonicalComponents says the payload *encodings* match the canonical set
	// too, not just the numbering. Knowing which id is which is not enough:
	// 1.21.11 writes an item nested in a component count-first where 26.1
	// writes it id-first, holds a registry reference in two bytes where 26.1
	// uses one, and writes a damage type tag as a bare string where 26.1
	// prefixes it. Same component, same name, different bytes.
	canonicalComponents bool
...
type VersionSpec struct {
	...
	// CanonicalComponents marks that this version's component payloads are
	// encoded the way understudy/components.go reads them. Set it only after
	// checking, because the check is not optional: see Version.ComponentKind.
	CanonicalComponents bool
...
func NewVersion(spec VersionSpec) *Version {
	return &Version{
		...
		canonicalComponents: spec.CanonicalComponents,
...
// CanonicalComponents reports whether this version encodes component payloads
// the way the decoder reads them.
//
// Separate from HasComponentIDs because the two drifted apart independently.
// 1.21.11's ids are known — generated from its own registries report — and its
// payloads still differ from 26.1's in at least nine components, so knowing
// which id is which buys nothing on its own.
func (v *Version) CanonicalComponents() bool { return v.canonicalComponents }
