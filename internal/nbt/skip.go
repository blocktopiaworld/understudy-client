...
//
// The compound and list cases are separate functions: they are the only two
// that recurse, and keeping them apart is what stops this becoming one long
// switch that nobody wants to re-read.
func skipPayload(data []byte, tag byte, depth int) (int, error) {
	...
	case TagLongArray:
		return array(data, 8)
	case TagList:
		return skipList(data, depth)
	case TagCompound:
		return skipCompound(data, depth)
	default:
		return 0, fmt.Errorf("nbt: unknown tag type %d", tag)
	}
}

// skipString steps over a length-prefixed string. The length is unsigned: a
// 40000-byte string is legal and a signed read would take it as negative.
...
// skipList steps over a typed list: an element type, a count, then that many
// payloads of the same type.
func skipList(data []byte, depth int) (int, error) {
	if len(data) < 5 {
	...
	off := 5
	// A non-positive count means an empty list, and the element type is then
	// meaningless — vanilla writes TAG_End for it.
...
// skipCompound steps over named entries until TAG_End.
func skipCompound(data []byte, depth int) (int, error) {
	off := 0
	for {
		if off >= len(data) {
		...
		// Entries inside a compound always carry their name, even when the
		// root does not.
		n, err := skipName(data[off:])
		if err != nil {
			return 0, err
		}
		off += n
